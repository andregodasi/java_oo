...
public class LivroDAO {
    ...
    private final String stmtListaLivroPorAutor = "select l.id,l.titulo from autor a\n" +
"inner join livro_autor la on a.id = la.idautor\n" +
"inner join livro l on l.id = idlivro where a.id = ?";
    ...
    public List<Livro> listarLivroComAutores() {
        ...
        try {
         ...
         public List<Livro> listarLivrosPorAutor(int id){
        Connection con = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            con = ConnectionFactory.getConnection();
            stmt = con.prepareStatement(stmtListaLivroPorAutor);
            stmt.setLong(1, id);
            rs = stmt.executeQuery();
            List<Livro> listaLivros = new ArrayList<Livro>();
            while (rs.next()) {
